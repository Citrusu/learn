define(function(require, exports, module){
    ...
    +function ($) {
        ...
        $.modal = function (params) {
            ...
            modalHTML = '<div class="ui-modal ' + extraClass + ' ' + noButtons + '">' +
                '<div class="ui-modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div>' +
                '<div class="ui-modal-buttons ' + verticalButtons + buttonClass + '">' + buttonsHTML + '</div>' +
                '</div>';
        ...
        $.prompt = function (text, title, callbackOk, callbackCancel) {
            ...
            return $.modal({
                ...
                onClick: function (modal, index) {
                    ...
                    if (index === 1 && callbackOk) callbackOk(modal,$(modal).find('.ui-modal-text-input').val());
                }
            });
        };
        $.modalLogin = function (options) {
            var usernamePlaceholder = options.usernamePlaceholder || '请输入用户名';
            var passwordPlaceholder = options.passwordPlaceholder || '请输入密码';
            return $.modal({
                text: options.text || '',
                title: options.title || '登录',
                afterText: '<input type="text" name="modal-username" placeholder="' + usernamePlaceholder + '" class="ui-modal-text-input ui-modal-text-input-double"><input type="password" name="modal-password" placeholder="' + passwordPlaceholder + '" class="ui-modal-text-input ui-modal-text-input-double">',
                buttons: [
                    {
                        text: defaults.modalButtonCancel
                    },
                ...
                onClick: function (modal, index) {
                    var username = $(modal).find('.ui-modal-text-input[name="modal-username"]').val();
                    var password = $(modal).find('.ui-modal-text-input[name="modal-password"]').val();
                    if (index === 0 && options.cancel) options.cancel(username, password);
                    if (index === 1 && options.ok)options.ok(modal,username, password);
                }
            });
        };
        $.modalPassword = function (options) {
            var passwordPlaceholder = options.passwordPlaceholder || '请输入密码';
            return $.modal({
                text: options.text || '',
                title: options.title || '登录',
                afterText: '<input type="password" name="modal-password" placeholder="' + passwordPlaceholder + '" class="ui-modal-text-input">',
                buttons: [
                    {
                        text: defaults.modalButtonCancel
                    },
                ...
                onClick: function (modal, index) {
                    var password = $(modal).find('.ui-modal-text-input[name="modal-password"]').val();
                    if (index === 0 && options.cancel) options.cancel(password);
                    if (index === 1 && options.ok) options.ok(modal,password);
                }
            });
        };
        ...
        $.darkPreloader = function (title,duration) {
            $.hidePreloader();
            $.showPreloader.preloaderModal = $.modal({
                extraClass:'ui-preloader-dark',
                title: title || defaults.modalPreloaderTitle,
                text: '<div class="ui-preloader"></div>'
            });
            if(duration){
        ...
        $.popup = function (modal, removeOnClose) {
            ...
            if (typeof modal === 'string' && modal.indexOf('<') >= 0) {
                ...
                if (_modal.childNodes.length > 0) {
                    ...
                    if (removeOnClose) modal.classList.add('ui-remove-on-close');
            ...
            modal.show();
            modal.find(".ui-content").scroller("refresh");
        ...
        $.pickerModal = function (pickerModal, removeOnClose) {
            ...
            if (typeof pickerModal === 'string' && pickerModal.indexOf('<') >= 0) {
                ...
                if (pickerModal.length > 0) {
                    if (removeOnClose) pickerModal.addClass('ui-remove-on-close');
        ...
        $.loginScreen = function (modal) {
            if (!modal) modal = '.ui-login-screen';
            modal = $(modal);
            if (modal.length === 0) return false;
            modal.show();
            if (modal.find('.' + defaults.viewClass).length > 0) {
        ...
        $.openModal = function (modal, cb,flag) {
            modal = $(modal);
        ...
        function handleClicks(e) {
            ...
            if (clicked.hasClass('ui-close-popup')) {
                ...
                else popup = '.ui-popup.ui-modal-in';
                $.closeModal(popup);
            }
